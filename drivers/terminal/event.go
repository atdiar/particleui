...
func withEventSupport[T ui.AnyElement](any T) T{
	...
	case DropDown:
		...
		k.SetSelectedFunc(func(text string, index int){
			...
			selected:= ui.NewEvent("selected", false, false, e, e,struct{string;int}{text,index}, o.Commit())
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
	...
	case InputField:
		...
		k.SetAcceptanceFunc(func(textToCheck string, lastChar rune) bool{
			...
			acceptance:= ui.NewEvent("acceptance", false, false, e, e,struct{textToCheck string; lastChar rune;}{textToCheck,lastChar}, o.Commit())
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
	...
	case List:
		...
		k.SetChangedFunc(func(index int, mainText string, secondaryText string, shortcut rune){
			changed:= ui.NewEvent("changed", false, false, e, e,struct{index int; main string; secondary string; shortcut rune;}{index,mainText,secondaryText,shortcut}, nil)
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
		...
		k.SetSelectedFunc(func(index int, mainText string, secondaryText string, shortcut rune){
			...
			selected:= ui.NewEvent("selected", false, false, e, e,struct{index int; main string; secondary string; shortcut rune;}{index,mainText,secondaryText,shortcut}, o.Commit())
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
	...
	case Modal:
		...
		k.SetDoneFunc(func(buttonIndex int, buttonLabel string){
			...
			done:= ui.NewEvent("done", false, false, e, e,struct{buttonIndex int; buttonLabel string;}{buttonIndex,buttonLabel}, o.Commit())
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
	...
	case Table:
		...
		k.SetSelectedFunc(func(row int, column int){
			...
			selected:= ui.NewEvent("selected", false, false, e, e,struct{row int; column int;}{row,column}, o.Commit())
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
		...
		k.SetSelectionChangedFunc(func(row int, column int){
			...
			selectionchanged:= ui.NewEvent("selected", false, false, e, e,struct{row int; column int;}{row,column}, o.Commit())
			defer func() {
				if r := recover(); r != nil {
					t:= tview.NewModal()
					app:= GetApplication(e).NativeElement()
